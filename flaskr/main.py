...
def getRecommendationBy(user_rates):
    ...
    if len(user_rates) > 0:
        ...
        # Predict ratings for all movies for the specified user
        user_id = get_current_user_id()
        ...
        # Select the top-100 items
        top_movie_ids = [pred.iid for pred in top_predictions[:100]]
        # Preserve the order of recommendations
        results = movies[movies['movieId'].isin(top_movie_ids)]
        results = results.set_index('movieId').loc[top_movie_ids].reset_index()
...
def getLikedSimilarBy(user_likes, user_dislikes=[]):
    results = []
    if len(user_likes) > 0 or len(user_dislikes) > 0:
        ...
        # print(item_rep_vector)
        # Step 2: Building user profile
        user_profile = build_user_profile(user_likes, user_dislikes, item_rep_vector, feature_list)
        # Step 3: Predicting user interest in items (generate top 100)
        results = generate_recommendation_results(user_profile, item_rep_matrix, item_rep_vector, 100)
    if len(results) > 0:
